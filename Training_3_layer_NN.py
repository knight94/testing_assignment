...
levels = {(0, 25): 0, (26, 50): 10, (51, 75): 20, (76, 100): 30, (101, 125):40, (126, 150):50, (151, 175):60, (176, 200): 70, (201, 225): 80, (226, 256): 90}
new_tr_img = np.zeros(train_images.shape)
# bins = [0, 50, 100, 150, 200, 256]
for i in range(train_images.shape[0]):
    # new_tr_img[i] = np.digitize(train_images[i], bins)
    for x in range(28):
        for y in range(28):
...
model_3.fit(new_tr_img, train_labels, epochs=1000, validation_split=0.2)


#Step 4 - Evaluate the model
test_loss, test_acc = model_3.evaluate(new_test_img, test_labels)
...
#     N = i
#     test_input = new_test_img[N].ravel().reshape(1,784)
    
#     hh_in = ((np.dot(test_input, ans[0]).transpose()).astype('int16') + ans[1].reshape(128,1)) / 32
#     hh_out = relu(hh_in)
    
#     out_in = ((np.dot(hh_out.transpose(), ans[2]).transpose()).astype('int16') + ans[3].reshape(10,1)) /32
...
for i in range(10000):
    ...
    hh_in = ((test_input.dot(ans[0]).transpose()) + ans[1].reshape(128,1)) / 32
    hh_out = relu(hh_in)
    
    out_in = ((hh_out.transpose().dot(ans[2]).transpose()) + ans[3].reshape(10,1)) /32
    # out_out = softmax_1(out_in)
    if (np.argmax(out_in) == test_labels[N]):
        count += 1
